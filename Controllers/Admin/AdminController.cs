...
namespace PageWhisphers.Controllers.Admin
{
    ...
    public class AdminController : Controller
    {
        ...
        public async Task<IActionResult> SendDeletionNotice(DeletionNoticeViewModel model)
        {
            ...
            try
            {
                ...
                await SendEmailAsync(
                    user.Email,
                    "Deletion Notice from PageWisphers",
                    $"<p>Dear {user.FirstName ?? "User"},</p>" +
                    $"<p>You have received a deletion notice from the PageWisphers Admin:</p>" +
                    $"<p>{model.Message}</p>" +
                    $"<p>Please respond to this email or take appropriate action to avoid account deletion.</p>" +
                    $"<p>Best regards,<br>PageWisphers Admin</p>"
                );
        ...
        public async Task<IActionResult> CancelDeletion(string id)
        {
            ...
            try
            {
                ...
                await SendEmailAsync(
                    user.Email,
                    "Deletion Notice Canceled - PageWisphers",
                    $"<p>Dear {user.FirstName ?? "User"},</p>" +
        ...
        public async Task<IActionResult> CreateStaff(string email, string password)
        {
            ...
            if (result.Succeeded)
            {
                ...
                try
                {
                    await SendEmailAsync(
                        email,
                        "Staff Account Created - PageWisphers",
                        $"<p>Dear Staff,</p>" +
